...
export function adventMain(input: string): any {
  let knots: [number, number][] = [
    ...
    [0, 0],
    [0, 0],
  ]; // Change the length of this array for Part 1 or Part 2!
    ...
    for (let i = 0; i < steps; i++) {
      ...
      for (let k = 1; k < knots.length; k++) {
        if (
          ...
          // If "tail" is not adjacent, move it.
          if (knots[k - 1]![0] !== knots[k]![0]) {
            knots[k]! = [
              knots[k]![0] +
                (knots[k - 1]![0] - knots[k]![0]) /
                  Math.abs(knots[k - 1]![0] - knots[k]![0]),
              knots[k]![1],
            ]; // If not in the same row, move 1 row closer
          }
            ...
            knots[k]! = [
              ...
              knots[k]![1] +
                (knots[k - 1]![1] - knots[k]![1]) /
                  Math.abs(knots[k - 1]![1] - knots[k]![1]),
            ]; // If not in the same col, move 1 col closer
          }
          if (
            k == tail &&
            !visited.some((location) => {
              return (
                location[0] === knots[k]![0] && location[1] === knots[k]![1]
              );
            })
          ) {
